...
def generate_motion_TDBY(Ss, S1, soil_class, max_iterations=10, max_non_improvement=2, duration=40.0, dt=0.01, pga_='tdby', omega_g=15.0, zeta_g=0.6, name='default', imageoutput=False, seed=None, damping=0.05, export_csv=True, export_json=True, export_sap2000=True, sap2000_two_column=False):
    ...
    Args:
        ...
        damping (float): Damping ratio (default: 0.05 = 5%)
        export_csv (bool): Whether to export CSV files for Excel/MATLAB
        export_json (bool): Whether to export structured JSON files
        export_sap2000 (bool): Whether to export SAP2000/ETABS import format
        sap2000_two_column (bool): Whether SAP2000 export should use two columns
    """
    ...
    spec_analysis.compute_response_spectrum(periods=periods, damping=damping)
    ...
    if imageoutput:
        ...
        # Calculate and print additional engineering metrics
        analyzer = SpectralAnalysis()
        analyzer.set_motion(t, results['acceleration_matched'])
        motion_params = analyzer.compute_ground_motion_parameters()
        print("\nFinal Ground Motion Parameters:")
        print(f"  Significant Duration (D5-95) : {motion_params['significant_duration_5_95']:.3f} s")
        print(f"  Significant Duration (D5-75) : {motion_params['significant_duration_5_75']:.3f} s")
        print(f"  Arias Intensity (Ia)         : {motion_params['arias_intensity_m_s']:.4f} m/s ({motion_params['arias_intensity_cm_s']:.2f} cm/s)")
        print(f"  Cumulative Absolute Velocity : {motion_params['cav_m_s']:.4f} m/s")
        print(f"  Peak Ground Velocity (PGV)   : {motion_params['pgv_m_s']:.4f} m/s")
        print(f"  Peak Ground Displacement(PGD): {motion_params['pgd_m']:.4f} m")
        print(f"  PGV / PGA Ratio              : {motion_params['pgv_pga_ratio']:.4f} s")
        print(f"  Time of Peak Acceleration    : {motion_params['t_pga']:.3f} s")
        print(f"  Zero-Crossing Rate           : {motion_params['zero_crossing_rate']:.2f} Hz")
    ...
    if imageoutput:
        # Output generator
        output_gen = OutputGenerator(output_dir=output_dir, damping=damping)
        output_gen.set_data(
            time=t,
        ...
        output_files = output_gen.generate_all_outputs(
            ...
            export_sap2000=export_sap2000,
            sap2000_two_column=sap2000_two_column
        )
    
        print(f"\nOutputs saved to '{os.path.abspath(output_dir)}' directory.")
        print(f"Report: {os.path.basename(output_files['report'])}")
        if 'acceleration_csv' in output_files:
            print(f"CSV Time History: {os.path.basename(output_files['acceleration_csv'])}")
            print(f"CSV Response Spectrum: {os.path.basename(output_files['spectrum_csv'])}")
        if 'json_data' in output_files:
            print(f"JSON Export Package: {os.path.basename(output_files['json_data'])}")
        if 'sap2000_data' in output_files:
            print(f"SAP2000 Import Format: {os.path.basename(output_files['sap2000_data'])}")
...
# Generate ground motion with custom damping ratio and all flexible formats enabled
best_avg_error, best_max_error = generate_motion_TDBY(
    Ss=1.3, S1=0.4, soil_class='ZB',
    max_iterations=30, max_non_improvement=15,
    duration=40.0, dt=0.01, pga_="tdby",
    omega_g=15.0, zeta_g=0.6, name='seeded',
    imageoutput=True, seed=3,
    damping=0.05,            # Custom damping ratio
    export_csv=False,         # Export CSV format
    export_json=True,        # Export JSON package
    export_sap2000=False,     # Export SAP2000 one-column format
    sap2000_two_column=False # Default to standard 1-column acceleration format
)
