...
class SpectralAnalysis:
    ...
    def compute_velocity_displacement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Applies a high-pass filter to both velocity and displacement to remove low-frequency
        integration drift (baseline drift).
        ...
        # Get baseline correction filter parameters to apply them to velocity and displacement
        bc_params = self.params.get('baseline_correction', {})
        cutoff_freq = bc_params.get('cutoff_freq', 0.1)
        filter_order = bc_params.get('filter_order', 4)
        ...
        v_raw = integrate.cumulative_trapezoid(self.accel_corrected, self.time, initial=0)
        # Apply highpass filter to remove integration drift
        self.velocity = signal.filtfilt(b, a, v_raw)
        
        # Calculate displacement: d(t) = ∫ v(t) dt
        d_raw = integrate.cumulative_trapezoid(self.velocity, self.time, initial=0)
        # Apply highpass filter to remove second integration drift
        self.displacement = signal.filtfilt(b, a, d_raw)
    ...
    def compute_ground_motion_parameters(self) -> Dict:
        """
        Calculate key ground motion parameters: Arias Intensity, CAV, Significant Duration, 
        and peak metrics (PGA, PGV, PGD, PGV/PGA ratio, zero-crossing rate).
        
        Returns:
            Dictionary containing computed ground motion metrics
        """
        self._check_motion()
        
        # Ensure baseline correction, velocity, and displacement are computed
        if self.accel_corrected is None:
            self.apply_baseline_correction()
        if self.velocity is None or self.displacement is None:
            self.compute_velocity_displacement()
        ...
        g = 9.81
        time = self.time
        accel_g = self.accel_corrected
        accel_m_s2 = accel_g * g
        
        # 1. Arias Intensity (Ia) in m/s and cm/s
        arias_profile = (np.pi / (2 * g)) * integrate.cumulative_trapezoid(accel_m_s2**2, time, initial=0)
        total_arias_m_s = arias_profile[-1]
        total_arias_cm_s = total_arias_m_s * 100.0
        
        # 2. Cumulative Absolute Velocity (CAV) in m/s
        cav_profile = integrate.cumulative_trapezoid(np.abs(accel_m_s2), time, initial=0)
        total_cav_m_s = cav_profile[-1]
        
        # 3. Significant Durations (D5-95 and D5-75)
        if total_arias_m_s > 0:
            percent_arias = arias_profile / total_arias_m_s
            idx_5 = np.where(percent_arias >= 0.05)[0][0]
            idx_75 = np.where(percent_arias >= 0.75)[0][0]
            idx_95 = np.where(percent_arias >= 0.95)[0][0]
            ...
            t_5, t_75, t_95 = 0.0, 0.0, 0.0
            d_5_95, d_5_75 = 0.0, 0.0
            
        # 4. Peak metrics
        pga_g = np.max(np.abs(accel_g))
        pga_m_s2 = pga_g * g
        t_pga = time[np.argmax(np.abs(accel_g))]
        
        pgv_m_s = np.max(np.abs(self.velocity)) * g
        pgd_m = np.max(np.abs(self.displacement)) * g
        
        # PGV/PGA ratio (s)
        pgv_pga_ratio = pgv_m_s / pga_m_s2 if pga_m_s2 > 0 else 0.0
        
        # 5. Nonstationarity / Zero Crossings
        zero_crossings = np.where(np.diff(np.sign(accel_g)))[0]
        num_zero_crossings = len(zero_crossings)
        zero_crossing_rate = num_zero_crossings / time[-1] if time[-1] > 0 else 0.0
        
        return {
            'arias_intensity_m_s': total_arias_m_s,
            'arias_intensity_cm_s': total_arias_cm_s,
            'cav_m_s': total_cav_m_s,
            't_5': t_5,
            't_75': t_75,
            't_95': t_95,
            'significant_duration_5_95': d_5_95,
            'significant_duration_5_75': d_5_75,
            'pga_g': pga_g,
            'pga_m_s2': pga_m_s2,
            't_pga': t_pga,
            'pgv_m_s': pgv_m_s,
            'pgd_m': pgd_m,
            'pgv_pga_ratio': pgv_pga_ratio,
    ...
    def get_analysis_data(self) -> Dict:
        ...
        ground_motion_params = self.compute_ground_motion_parameters()
        ...
        return {
            ...
            'ground_motion_parameters': ground_motion_params
        }